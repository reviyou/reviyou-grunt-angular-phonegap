...
module.exports = (grunt)->
  @initAngularPhonegapConfig = ->
   ...
   grunt.registerTask 'phonegap:buildDebug', 'Build for phonegap (use `build:phonegap:[platform]` when not android)', (target="android")->
      grunt.task.run ['buildDebug', 'clean:phonegap', 'copy:phonegap', "shell:phonegapBuild:#{target}"]
  ...
  if _.isEmpty(grunt.config.data)
    grunt.verbose.writeln "Call reviyou-grunt-angular-phonegap initialization #{"LATER".green.bold}"
    ...
    grunt.verbose.writeln "Call reviyou-grunt-angular-phonegap initialization #{"NOW".green.bold}"
    @initAngularPhonegapConfig()